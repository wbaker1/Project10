...
int main() {
    ...
    glBindVertexArray(0); // Unbind VAO
    	...
    	unsigned char* image = SOIL_load_image("posx.jpg", &width, &height, 0, SOIL_LOAD_RGB);
    	...
    	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    	// Load, create texture and generate mipmaps
    	image = SOIL_load_image("Bump-Map.jpg", &width, &height, 0, SOIL_LOAD_RGB);
    	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
    	glGenerateMipmap(GL_TEXTURE_2D);
    	SOIL_free_image_data(image);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        /*
       	If cant figure out turn in this
        // Bind Textures using texture units
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture1);
        glUniform1i(glGetUniformLocation(cylinderShader.Program, "ourTexture2"), 2);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture3);
        glUniform1i(glGetUniformLocation(sphereShader.Program, "ourTexture2"), 2);  
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, texture2);
        glUniform1i(glGetUniformLocation(cubeShader.Program, "ourTexture2"), 2);  
        */
       // Bind Textures using texture units
        ...
        for (int i = 0; i < 8; i++) { // For 8 rows
            for (int j = 0; j < 8; j++) { // For 8 columns
                ...
                view_square = glm::translate(view_square, glm::vec3(-4.0f, -0.5f, -9.0f)); // Translate square to posiiton [setting x and z for grid]
        ...
        view_cube = glm::translate(view_cube, glm::vec3(-0.15f, 0.1f, -5.0f)); // Translate cube back
        view_cube = glm::scale(view_cube, glm::vec3(1.5f, 1.5f, 1.5f)); // Scale down sphere
        ...
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, texture3);
        glUniform1i(glGetUniformLocation(cubeShader.Program, "ourTexture3"), 0);
        ...
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture1);
        glUniform1i(glGetUniformLocation(cylinderShader.Program, "ourTexture1"), 2);
        ...
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture2);
        glUniform1i(glGetUniformLocation(sphereShader.Program, "ourTexture2"), 1); 
